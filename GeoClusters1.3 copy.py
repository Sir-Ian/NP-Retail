...
# Normalize the "Sum of Amount (Net)" column
scaler = StandardScaler()
data['normalized_amount'] = scaler.fit_transform(data[['Sum of Amount (Net)']])

# Assign weights based on the normalized amounts (adjust the strategy as needed)
data['weight'] = np.maximum(data['weight'], 0)  # Set negative weights to 0

# Prepare data for clustering (select relevant columns)
clustering_data = data[['latitude', 'longitude', 'weight']]

# Define the number of clusters (adjust as needed)
k = 15

# Create a weighted K-means model
weighted_kmeans = KMeans(n_clusters=k, init='k-means++', max_iter=300, n_init=10, algorithm='elkan')

# Fit the model to the weighted data
weighted_kmeans.fit(clustering_data, sample_weight=data['weight'])

# Assign cluster labels to the data
data['weighted_cluster'] = weighted_kmeans.labels_
...
threshold_distance = 100  # Distance in miles
...
data_filtered = data[data['distance_to_retail'] > threshold_distance]
...
data_filtered['cluster'] = db.labels_
...
clusters, counts = np.unique(data['weighted_cluster'][data['weighted_cluster'] != -1], return_counts=True)
...
for i, cluster in enumerate(top_clusters):
    cluster_mask = (data['weighted_cluster'] == cluster)
    cluster_data = data[cluster_mask]
    plt.plot(cluster_data['longitude'], cluster_data['latitude'], 'o', markerfacecolor=top_colors[i], markeredgecolor='k', markersize=5, label=f'Weighted Cluster {cluster}')
